...
- **`categories` cannot be DELETEd, TRUNCATEd, upserted, or renamed — a live catalog defect from migration 0011.** 0011 rebuilt `transaction_overrides` through a `transaction_overrides_new` staging table and renamed it; the rename left `categories` holding a foreign-key dependency record still naming the staging table. Every statement that makes DuckDB validate a constraint on `categories` dereferences it and fails with `Catalog Error: Table with name transaction_overrides_new does not exist!`. This is persisted in the catalog — closing and reopening the database does not clear it. Plain `INSERT` and `UPDATE`s that touch no constrained column still work, which is why `set_category_spending` is fine and why nobody has hit this: no surface deletes or renames a category. Restore works inside those bounds (`reconcile_table`) and reports what it could not write. **A proper fix needs a schema-repair migration that rebuilds `categories`, which means dropping and recreating its four dependents (`category_rules`, `transaction_overrides`, `goals`, `category_match_cache`) — real user state, so it deserves its own slice with the pre-migration mitigation discipline above.** Until then, don't add a `category delete` or `category rename` command; it will fail at runtime.
