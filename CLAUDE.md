...
- **Cap any dependency whose import path we rely on, and declare security floors where the audit can see them.** Two failures on 2026-08-11, both of which took every open PR red at once, neither caused by a code change:

  1. **An uncapped major bump breaks the repo overnight.** `mcp>=1.2` resolved to `mcp` 2.0, which removed `mcp.server.fastmcp` — the module `server.py`, `daemon.py` and `web/app.py` all import. Every CI job failed at import, on branches nobody had touched in weeks, and `pip install -e .` produced a broken install for any new contributor. The rule: if we import a specific module path from a dependency, pin below its next major (`mcp>=1.28.1,<2`) and treat lifting the cap as a port with its own slice. A floor alone is not enough.
  2. **`[build-system].requires` never reaches the environment `pip-audit` scans.** That table only provisions the isolated *build* env. `pip install -e ".[dev]"` builds a venv where setuptools comes from the Python install (on CI, `actions/setup-python`), so the `setuptools>=78.1.1` pin added by the 2026-05 audit did nothing for months while looking like a fix — the advisory was still open and nobody could tell. Security floors on transitive/tooling packages must ALSO be declared in `[project.optional-dependencies].dev` (or wherever the audited env resolves them). Verify a remediation by re-running the scanner in the environment it actually inspects, not by reading the constraint back.

  Corollary for CI: `security.yml` installs a freshly resolved set on Linux. It audits *the repo*, never the long-lived local venv the daemon runs from, and cannot see a Windows-only advisory. CI green does not mean the running deployment is patched.
- **Don't assert on rendered CLI output as a literal substring.** Typer/Rich wrap usage errors into a bordered panel sized to the terminal, and CI's Linux runners are narrow enough to break *inside a word* — `"mutually exclusive"` arrives as `mutually excl`, a box border, then `usive`. Neither a plain `in` check nor whitespace normalization survives it, and the failure is platform-dependent (Windows runners passed, Linux failed), which makes it look like a real bug. Reduce the output to letters and digits before matching (`tests/test_import_cli.py::_letters`), and reproduce locally with `COLUMNS=40 pytest`.
...
- **Pending rows are a snapshot — don't turn them into history, and don't move the reconcile.** `pending=TRUE` rows live in the same `transactions` table but follow different rules: each successful sync replaces the pending set via `delete_stale_pending` (accounts whose response entry carried a transactions *list*, ids not seen in this sync's union). Four invariants, each pinned by a test in `test_collector.py`/`test_duckdb_store.py`: (1) reconcile runs ONCE per `collect()` over the union of all chunks' ids — per-chunk would delete still-pending rows while processing older windows; (2) never reconcile after a failed sync — a partial feed view must not delete anything; (3) the delete is deliberately NOT date-scoped — a long-pending hold temporarily vanishing beats a settled-under-new-id hold double-counting forever; (4) accounts with `"transactions": null` in the response are excluded from scope (institution hiccup ≠ empty feed). Pending ids are bank-unstable, which is also why per-id overrides on pending rows carry a caveat (both surfaces) and why `delete_stale_pending` removes the orphaned override along with the row. If you need "when did this hold first appear" history, that's a new feature (an events table), not a relaxation of the snapshot contract.
- **Don't add a `category_id` column to `transactions`.** Resolution is read-time via the `transactions_with_category` view on purpose — adding or editing rules / overrides applies retroactively without any backfill. A write-time column would silently break that contract, force a backfill on every rule change, and defeat the "rules-and-overrides" design of the categorization slice. The pydantic `Transaction` model does NOT carry a `category` field for the same reason; only `serialize_transaction` (the dict shape sent over MCP) attaches a resolved category.
  ...
  Since migration 0013 the *rule match* behind that view is cached in `category_match_cache` — this is NOT the forbidden write-time column; it is derived bookkeeping the store maintains write-through so reads stop paying O(transactions × rules) pattern matching (pre-0013 the match was O(transactions × rules) per scan, costing a large fraction of a second of one core on a multi-year database PER VIEW SCAN, and /goals scanned once per cap). The contract holds because every mutation of the matching inputs updates the cache inside the same store call: `upsert_transactions` refreshes the upserted ids, `delete_stale_pending` deletes theirs, `add_rule`/`remove_rule` rebuild in the same transaction as the rule write (the rebuild IS the retroactive application), and `init()` rebuilds after applying any migration. Overrides never touch the cache — the view still resolves them read-time. **Invariant: any new write path that touches `transactions` rows, `category_rules`, or matching semantics must maintain the cache** (targeted `_refresh_category_match_cache` for row-scoped writes, `rebuild_category_match_cache()` for anything rule-shaped) — a missed path means silently wrong categories, not an error. The match SQL has one home: `_match_cache_insert_sql` in `duckdb_store.py`. Out-of-band bulk loads (in-engine INSERTs in tests/scripts) must call `store.rebuild_category_match_cache()` afterward.
- **Don't "simplify" the backup into a file copy or an `EXPORT DATABASE`.** Both were measured and both are unavailable in the case that matters (daemon running, which is when an automated backup must work). (1) DuckDB opens the database file with a share mode that denies every other reader: `shutil.copy2` on the live `data.duckdb` raises `PermissionError` even from *inside the process holding the handle*, so there is no file-copy path, and closing/reopening isn't an option for a live daemon. (2) The store's connection sets `enable_external_access=false`, which is immutable once the database is running and blocks `EXPORT DATABASE`, `COPY ... TO`, and `ATTACH 'backup.duckdb'` — all four verified to raise `PermissionException`; only `CHECKPOINT` survives. That is why `backup.py` pulls rows out through the open connection as Python objects and writes JSON Lines itself. The archive being engine-independent is a bonus, not the motivation. Verification is not decorative either: the zip is staged, test-restored, and checked (checksums, row counts, summed amounts) *before* `os.replace` moves it into the destination, so a cloud-sync client never sees an archive that doesn't restore.

- **A backup archive is untrusted input. Treat every identifier in it as hostile.** Restore reads column names out of the archive's own JSONL header and interpolates them as SQL identifiers, because a column name cannot be bound as a parameter. Quoting is not sufficient: a name containing a double quote closes the identifier early and the remainder parses as SQL, which DuckDB executes because its `execute()` accepts multiple statements. Confirmed exploitable by PoC in the 2026-08 audit (finding 1) — a tampered archive achieved arbitrary SQL against the restored database; `enable_external_access=false` still held, so it could not reach the filesystem or network, but it could rewrite any financial row.

  `DuckDBStore._assert_known_columns` checks every archive column against `DESCRIBE` before any statement is built, so only identifiers the live table already has can reach SQL. **Any new code path that takes a name from archive content and interpolates it must go through that check**, and the guard also produces the right error for the ordinary case (an archive written against a schema this install lacks says so, instead of dying in the parser). The threat is not paranoid: `backup.py`'s documented workflow points the destination at a cloud-synced folder, so archives live somewhere a second party can write.

  Note the shape of the near-miss: the eight `# noqa: S608` sites carried a rationale about the *table* name being safe (it comes from `RESTORE_ORDER`) and said nothing about the *column* names interpolated on the same line. The suppression wasn't wrong, it was incomplete. A `noqa`/`nosec` rationale must account for every interpolated value on the line.

- **DuckDB sequences cannot be repositioned; restore burns them forward.** `CREATE OR REPLACE SEQUENCE` and `DROP SEQUENCE` raise `DependencyException` when a column DEFAULT references the sequence, and `ALTER SEQUENCE ... RESTART` is `NotImplementedException` (DuckDB 1.5.4). `DuckDBStore.advance_sequence` therefore consumes values with `SELECT nextval('seq') FROM range(N)` — one statement, not N. Skipping this leaves a restored database handing out id 1 while the restored rows already occupy that id and everything above it, so the first new rule/category/goal after a restore dies on the primary key. `_SEQUENCE_OWNERS` maps each sequence to the table it feeds; extend it when a migration adds a sequence.

- **Never rebuild a table with create-copy-drop-RENAME. Use create-under-the-final-name.** `ALTER TABLE x_new RENAME TO x` leaves DuckDB's foreign-key metadata on the *parent* side still naming `x_new`. Every later statement that makes DuckDB validate a constraint on that parent dereferences the vanished name and fails with `Catalog Error: Table with name x_new does not exist!` — DELETE of one row, DELETE of all rows, TRUNCATE, `ON CONFLICT DO UPDATE`, and UPDATE of a UNIQUE column, *even for a parent row nothing references*. The record is persisted; closing and reopening does not clear it. Plain INSERT and UPDATEs touching no constrained column still work, which is what makes it so easy to miss.

  Migration 0011 did this to `categories` (via `transaction_overrides_new`) and it went unnoticed for five migrations, because no surface deletes or renames a category — it only surfaced when backup/restore needed to clear the table. 0014 documents the same class of bug hitting `accounts` and deliberately used the safe shape: back the rows up into a constraint-free scratch table, DROP the old table, CREATE under the final name, copy back, drop the scratch. Follow 0014, not 0011.

  **Migration 0016 repairs 0011's damage** by recreating a table under the name the catalog still believes in and then DROPping it — DROP is the clean removal path the rename bypassed. If you ever hit this again, that two-statement repair is the fix; you do not need to rebuild the parent table. What 0016 does *not* change: a category that rules, overrides or goals reference still cannot be deleted or renamed, because that is DuckDB's ordinary FK rule (an UPDATE of a UNIQUE column is internally a delete+insert, so it trips the same parent-side check). A `category delete` / `category rename` command is now possible but needs the same reference guard `delete_account` already has.
